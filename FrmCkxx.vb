...
#Region "定义变量"
    ...
    ReadOnly dtCk As New DataTable("rc_ckxx")
    '打印文档
...
#Region "初始化"
    ...
    Private Sub FrmCkxx_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load

        '数据绑定
        dtCk.Columns.Add("ckdm", Type.GetType("System.String"))
        ...
        Finally
            rcOleDbConn.Close()
        End Try
        rcDataView = New DataView(rcDataset.Tables("rc_ckxx"))
...
#Region "打印、打印预览"
    ...
    Private Sub PreparePrintData()
        ...
        Dim i As Integer
        For i = 0 To rcDataset.Tables("rc_ckxx").Rows.Count - 1
            rcRps.Text(i + 1, 1) = Trim(rcDataset.Tables("rc_ckxx").Rows(i).Item("ckdm"))
            rcRps.Text(i + 1, 2) = Trim(rcDataset.Tables("rc_ckxx").Rows(i).Item("ckmc"))
            rcRps.Text(i + 1, 3) = Trim(rcDataset.Tables("rc_ckxx").Rows(i).Item("cksm"))
        Next
        '取RPS数据
        Try
            rcOleDbConn.Open()
...
#Region "输出"
    ...
    Private Sub BtnExport_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnExport.Click, MnuiExport.Click
        '导出数据
        Exports2Excel(rcDataset.Tables("rc_ckxx"))
    End Sub
...
#Region "删除"
    ...
    Private Sub BtnDelete_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnDelete.Click, MnuiDelete.Click
        '删除
        '删除数据
        If MessageBox.Show("您真地要删除吗？" & Trim(BindingContext(rcDataView, "").Current("ckdm")), "提示信息", MessageBoxButtons.OKCancel, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) = DialogResult.OK Then
            currentPos = BindingContext(rcDataView, "").Position
            If Trim(BindingContext(rcDataView, "").Current("ckdm")) = "" Then
            ...
            Try
                ...
                rcOleDbCommand.CommandTimeout = 300
                rcOleDbCommand.CommandType = CommandType.Text
                rcOleDbCommand.CommandText = "DELETE FROM rc_ckxx WHERE ckdm = ?"
                rcOleDbCommand.Parameters.Clear()
                rcOleDbCommand.Parameters.AddWithValue("@ckdm", BindingContext(rcDataView, "").Current("ckdm"))
