    ...
    Private Sub TsbSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TsbSave.Click
        ...
        If Me.RadioButton1.Checked Then
            Try
                ...
                '删除已保存的数据
                rcOleDbCommand.CommandText = "DELETE FROM inv_dbd WHERE SUBSTR(djh,1,10) = ?"
        ...
        For i = 0 To rcDataset.Tables("result").Rows.Count - 1
            Try
                ...
                rcOleDbCommand.Parameters.Add("@paraDateDbrq", OleDbType.Date, 8).Value = rcDataset.Tables("result").Rows(i).Item("调拨日期")
                rcOleDbCommand.Parameters.Add("@paraBlnDelete", OleDbType.Numeric, 1).Value = 0
                rcOleDbCommand.Parameters.Add("@paraStrCckdm", OleDbType.VarChar, 12).Value = rcDataset.Tables("result").Rows(i).Item("调出仓编码")
                rcOleDbCommand.Parameters.Add("@paraStrCckmc", OleDbType.VarChar, 30).Value = rcDataset.Tables("result").Rows(i).Item("调出仓名称")
                rcOleDbCommand.Parameters.Add("@paraStrRckdm", OleDbType.VarChar, 12).Value = rcDataset.Tables("result").Rows(i).Item("调入仓编码")
                rcOleDbCommand.Parameters.Add("@paraStrRckmc", OleDbType.VarChar, 30).Value = rcDataset.Tables("result").Rows(i).Item("调入仓名称")
                ...
                rcOleDbCommand.Parameters.Add("@paraStrPiHao", OleDbType.VarChar, 20).Value = ""
                ...
                rcOleDbCommand.Parameters.Add("@paraStrDbmemo", OleDbType.VarChar, 50).Value = rcDataset.Tables("result").Rows(i).Item("备注")
            ...
            End Try
        Next
        MsgBox("物料调拨单读入完成,请检查数据。", MsgBoxStyle.OkOnly + MsgBoxStyle.Question, "提示信息")
        Me.Close()
    End Sub
