...
#Region "定义变量"
    ...
    ReadOnly dtBm As New DataTable("rc_jldw")
    '打印文档
...
#Region "初始化"
    ...
    Private Sub FrmJldwxx_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load

        '数据绑定
        dtBm.Columns.Add("jldwdm", Type.GetType("System.String"))
        dtBm.Columns.Add("jldwmc", Type.GetType("System.String"))
        dtBm.Columns.Add("jldwsm", Type.GetType("System.String"))
        dtBm.Columns.Add("xsws", Type.GetType("System.String"))
        rcDataset.Tables.Add(dtBm)
        With dtBm
            .Columns("jldwdm").DefaultValue = ""
            .Columns("jldwmc").DefaultValue = ""
            .Columns("jldwsm").DefaultValue = ""
            .Columns("xsws").DefaultValue = 0
        End With
        '显示等待样式鼠标
        ...
        Finally
            rcOleDbConn.Close()
        End Try
        rcDataView = New DataView(rcDataset.Tables("rc_jldw"))
...
#Region "打印、打印预览"
    ...
    Private Sub PreparePrintData()
        ...
        Dim i As Integer
        For i = 0 To rcDataset.Tables("rc_jldw").Rows.Count - 1
            rcRps.Text(i + 1, 1) = Trim(rcDataset.Tables("rc_jldw").Rows(i).Item("jldwdm"))
            rcRps.Text(i + 1, 2) = Trim(rcDataset.Tables("rc_jldw").Rows(i).Item("jldwmc"))
            rcRps.Text(i + 1, 3) = Trim(rcDataset.Tables("rc_jldw").Rows(i).Item("jldwsm"))
        Next
        '取RPS数据
        Try
            rcOleDbConn.Open()
...
#Region "输出"
    ...
    Private Sub BtnExport_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnExport.Click, MnuiExport.Click
        '导出数据
        Exports2Excel(rcDataset.Tables("rc_jldw"))
    End Sub
...
#Region "删除"
    ...
    Private Sub BtnDelete_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnDelete.Click, MnuiDelete.Click
        '删除
        '删除数据
        If MessageBox.Show("您真地要删除吗？" & Trim(BindingContext(rcDataView, "").Current("jldwdm")), "提示信息", MessageBoxButtons.OKCancel, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) = DialogResult.OK Then
            currentPos = BindingContext(rcDataView, "").Position
            If Trim(BindingContext(rcDataView, "").Current("jldwdm")) = "" Then
            ...
            Try
                ...
                rcOleDbCommand.CommandTimeout = 300
                rcOleDbCommand.CommandType = CommandType.Text
                rcOleDbCommand.CommandText = "DELETE FROM rc_jldw WHERE jldwdm = ?"
                rcOleDbCommand.Parameters.Clear()
                rcOleDbCommand.Parameters.AddWithValue("@jldwdm", BindingContext(rcDataView, "").Current("jldwdm"))
