Public Class FrmJtchdjzbz

    Private Sub FrmJtchdjzbz_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Dim i As Integer
        Me.DgtbcKcsl.Format = g_FormatSl0
        ...
        For i = 0 To ParaDataSet.Tables("rc_kczlfd").Rows.Count - 1
            ...
            Dim DgtbcKcje As New DataGridTextBoxColumn With {
            ...
            Dim DgtbcJitibilv As New DataGridTextBoxColumn With {
                .MappingName = "jitibilv_" & (i + 1).ToString.PadLeft(2, "0")
            }
            If i = paraDataSet.Tables("rc_kczlfd").Rows.Count - 1 And i > 0 Then
                DgtbcJitibilv.HeaderText = paraDataSet.Tables("rc_kczlfd").Rows(i - 1).Item("zhangling") & "个月以上计提比例"
            Else
                DgtbcJitibilv.HeaderText = paraDataSet.Tables("rc_kczlfd").Rows(i).Item("zhangling") & "个月计提比例"
            End If
            DgtbcJitibilv.Format = g_FormatJe0
            DgtbcJitibilv.Alignment = HorizontalAlignment.Right
            DgtbcJitibilv.Width = 80
            Me.DataGridTableStyle1.GridColumnStyles.Add(DgtbcJitibilv)
            Dim DgtbcJitije As New DataGridTextBoxColumn With {
                .MappingName = "jitije_" & (i + 1).ToString.PadLeft(2, "0")
            }
            If i = paraDataSet.Tables("rc_kczlfd").Rows.Count - 1 And i > 0 Then
                DgtbcJitije.HeaderText = paraDataSet.Tables("rc_kczlfd").Rows(i - 1).Item("zhangling") & "个月以上计提金额"
            Else
                DgtbcJitije.HeaderText = paraDataSet.Tables("rc_kczlfd").Rows(i).Item("zhangling") & "个月计提金额"
            End If
            DgtbcJitije.Format = g_FormatJe0
            DgtbcJitije.Alignment = HorizontalAlignment.Right
            DgtbcJitije.Width = 80
            Me.DataGridTableStyle1.GridColumnStyles.Add(DgtbcJitije)
        Next
    ...
    Private Sub PreparePrintData()
        ...
        Try
            rcOleDbCommand.CommandText = "SELECT * FROM rc_rps where rpsid = 'CpkcZlfx'"
        ...
        '套打
        'rcRps.PaperType = 1
        rcRps.Text(-1, 1) = "物料账面收发存汇总表" & "（" & Trim(Label3.Text) & "）"
