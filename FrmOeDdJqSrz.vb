...
Public Class FrmOeDdJqSrz
    ...
    Private Sub FrmOeDdJqSrz_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
        '设置DataGridView
        ...
        For i = 0 To rcDataSet.Tables("rc_ddnr").Rows.Count - 1
            If rcDataSet.Tables("rc_ddnr").Rows(i).Item("scjhrq").GetType.ToString <> "System.DBNull" And rcDataSet.Tables("rc_ddnr").Rows(i).Item("sczydm").GetType.ToString <> "System.DBNull" Then
                Try
                    ...
                    '    rcOleDbCommand.CommandText = "UPDATE rc_cpxx SET bzcb = ? WHERE cpdm = ?"
                    '    rcOleDbCommand.Parameters.Clear()
                    ...
                    'End If
                    rcOleDbCommand.CommandText = "UPDATE oe_dd SET sczydm = ?,sczymc = ?,scjhrq = ?,fhrq = ? WHERE djh = ? and xh = ? and cpdm = ?"
                    rcOleDbCommand.Parameters.Clear()
                    ...
                    rcOleDbCommand.Parameters.Add("@fhrq", OleDbType.Date, 8).Value = rcDataSet.Tables("rc_ddnr").Rows(i).Item("scjhrq")
                    ...
                    '                If rcDataSet.Tables("rc_cpxx").Rows(0).Item("bzcb") <> rcDataSet.Tables("rc_ddnr").Rows(i).Item("bzcb") Then
