...
#Region "初始化"
    ...
    Private Sub FrmOeXsdSrz_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            ...
            rcOleDbCommand.CommandTimeout = 300
            rcOleDbCommand.CommandType = CommandType.Text
            rcOleDbCommand.CommandText = "SELECT pzlxdm,pzlxjc FROM rc_lx WHERE lxgs = '产品送货单' and kjnd = '" & strYear & "' ORDER BY pzlxdm"
            rcOleDbCommand.Parameters.Clear()
            rcOleDbDataAdpt.SelectCommand = rcOleDbCommand
            ...
            If rcDataset.Tables("rc_pzno").Rows(0).Item("pzno") >= Mid(Trim(Me.TxtDjh.Text), 15, 5) Then
                ...
                Try
                    ...
                    If rcDataset.Tables("rc_xsdml").Rows.Count > 0 Then
                        ...
                        Next
                        If blnFp Then
                            MsgBox("该单据已经开票，不能修改。", MsgBoxStyle.OkOnly + MsgBoxStyle.Question, "提示信息")
...
#Region "新单事件"
    ...
    Private Sub ShowNewDjh()
        ...
        Try
            ...
            rcOleDbCommand.CommandTimeout = 300
            rcOleDbCommand.CommandType = CommandType.Text
            rcOleDbCommand.CommandText = "SELECT pzlxdm,pzno" & strMonth & " as pzno,bfushu FROM rc_lx WHERE pzlxdm = ? and lxgs = '产品送货单' and kjnd = '" & strYear & "'"
            rcOleDbCommand.Parameters.Clear()
...
#Region "保存事件"
    ...
    Private Sub SaveEvent(ByVal blnPrint As Boolean)
            ...
            For i = 0 To rcDataset.Tables("rc_xsdnr").Rows.Count - 1
                If rcDataset.Tables("rc_xsdnr").Rows(i).RowState <> DataRowState.Deleted Then
                    ...
                    If rcDataset.Tables("cpdmcnt").Rows.Count <> 1 Then
                        MsgBox(Trim(rcDataset.Tables("rc_xsdnr").Rows(i).Item("cpdm")) & "产品编码不正确，请检查。", MsgBoxStyle.OkOnly + MsgBoxStyle.Exclamation, "提示信息")
                        Return
                    End If
                    If rcDataset.Tables("rc_xsdnr").Rows(i).Item("sl").GetType.ToString <> "System.DBNull" Then
        ...
        For i = 0 To dtCopy.Rows.Count - 1
            If Not String.IsNullOrEmpty(dtCopy.Rows(i).Item("dddjh")) Then
                ...
                If dblWrksl >= 0 And dtCopy.Rows(i).Item("sl") > 0 And dblWrksl < dtCopy.Rows(i).Item("sl") Then
                    MsgBox("该物料的销售订单可送货数量小于送货数量，请检查。" & Chr(13) & "物料编码：" & dtCopy.Rows(i).Item("cpdm") & Chr(13) & "订单单据号：" & dtCopy.Rows(i).Item("dddjh") & Chr(13) & "订单行号：" & dtCopy.Rows(i).Item("ddxh") & Chr(13) & "当前送货数量：" & dtCopy.Rows(i).Item("sl").ToString & Chr(13) & "可送货数量：" & dblWrksl.ToString, MsgBoxStyle.OkOnly + MsgBoxStyle.Exclamation, "提示信息")
                    Return
                End If
                If dblWrksl < 0 And dtCopy.Rows(i).Item("sl") < 0 And dblWrksl > dtCopy.Rows(i).Item("sl") Then
                    MsgBox("该物料的订单可送货数量小于送货数量，请检查。" & Chr(13) & "物料编码：" & dtCopy.Rows(i).Item("cpdm") & Chr(13) & "订单单据号：" & dtCopy.Rows(i).Item("dddjh") & Chr(13) & "订单行号：" & dtCopy.Rows(i).Item("ddxh") & Chr(13) & "当前送货数量：" & dtCopy.Rows(i).Item("sl").ToString & "可送货数量：" & dblWrksl.ToString, MsgBoxStyle.OkOnly + MsgBoxStyle.Exclamation, "提示信息")
                    Return
                End If
            End If
        Next
        ...
        '            MsgBox("该物料的订单可送货数量小于送货数量，请检查。" & rcDataSet.Tables("rc_xsdnr").Rows(i).Item("cpdm"), MsgBoxStyle.OkOnly + MsgBoxStyle.Exclamation, "提示信息")
        '            Return
        '        End If
        '        If dblWrksl < 0 And rcDataSet.Tables("rc_xsdnr").Rows(i).Item("sl") < 0 And dblWrksl > rcDataSet.Tables("rc_xsdnr").Rows(i).Item("sl") Then
        '            MsgBox("该物料的订单可送货数量小于送货数量，请检查。" & rcDataSet.Tables("rc_xsdnr").Rows(i).Item("cpdm"), MsgBoxStyle.OkOnly + MsgBoxStyle.Exclamation, "提示信息")
    ...
    Private Sub PreparePrintData()
        ...
        Select Case True
            ...
            Case strRpsId = "SHD_DC"
                rcRps.Text(-1, 1) = g_PrnDwmc & "送货凭据"
                rcRps.Text(-1, 2) = "客户：" & "(" & Trim(Me.TxtKhdm.Text) & ")" & Trim(Me.LblKhmc.Text)
                rcRps.Text(-1, 3) = "日期：" & Me.DtpXsrq.Value.Date.ToLongDateString
                ...
                If Decimal.op_Modulus(rcDataSet.Tables("rc_xsdnr").Rows.Count, rcRps.LinesPerPage.ToString) <> 0 Then
                    ...
                    Next
                End If
                rcRps.Text(-1, 23) = "开票人：" & g_User_DspName
            Case strRpsId = "XSDBZ"
            Case strRpsId = "XSDPT"
            Case Else
                MsgBox("s")
