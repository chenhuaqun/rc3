...
#Region "初始化"
    ...
    Private Sub FrmOeYpFhdhSrz_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        '设置DataGridView
        Me.rcDataGridView.AutoGenerateColumns = False
        Me.rcDataGridView.ColumnHeadersDefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter
        Me.rcDataGridView.Columns("ColSl").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
        Me.rcDataGridView.Columns("ColSl").DefaultCellStyle.Format = g_FormatInt0
        ...
        For i = 0 To rcDataSet.Tables("rc_ypddnr").Rows.Count - 1
            If rcDataSet.Tables("rc_ypddnr").Rows(i).Item("xz") Then
                Try
                    ...
                    rcOleDbCommand.CommandText = "UPDATE oe_ypdd SET zt = 4 ,shdh = ? WHERE xh = ? and djh = ?"
                    rcOleDbCommand.Parameters.Clear()
                    rcOleDbCommand.Parameters.Add("@shdh", OleDbType.VarChar, 30).Value = rcDataSet.Tables("rc_ypddnr").Rows(i).Item("shdh")
            ...
            Else
                Try
                    ...
                    rcOleDbCommand.Connection = rcOleDbConn        ' Assign transaction object for a pending local transaction
                    rcOleDbCommand.Transaction = rcOleDbTrans
                    rcOleDbCommand.CommandText = "UPDATE oe_ypdd SET zt = 3 ,shdh = NULL WHERE xh = ? AND djh = ?"
                    rcOleDbCommand.Parameters.Clear()
                    rcOleDbCommand.Parameters.Add("@xh", OleDbType.Integer, 4).Value = rcDataSet.Tables("rc_ypddnr").Rows(i).Item("xh")
                    rcOleDbCommand.Parameters.Add("@djh", OleDbType.VarChar, 15).Value = rcDataSet.Tables("rc_ypddnr").Rows(i).Item("djh")
                    rcOleDbCommand.ExecuteNonQuery()
