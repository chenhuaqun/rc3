...
#Region "初始化"
    ...
    Private Sub FrmOeYpFhrqSrz_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        '设置DataGridView
    ...
    Private Sub BtnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnSave.Click
        ...
        For i = 0 To rcDataSet.Tables("rc_ypddnr").Rows.Count - 1
            If rcDataSet.Tables("rc_ypddnr").Rows(i).Item("xz") Then
                ...
                Try
                    rcOleDbCommand.CommandText = "UPDATE oe_ypdd SET fhrq = ?,zl = ?,fhrqsj = SYSDATE  WHERE djh = ? and xh = ?"
                    rcOleDbCommand.Parameters.Clear()
                    rcOleDbCommand.Parameters.Add("@fhrq", OleDbType.Date, 8).Value = rcDataSet.Tables("rc_ypddnr").Rows(i).Item("fhrq")
                    rcOleDbCommand.Parameters.Add("@zl", OleDbType.Numeric, 14).Value = rcDataSet.Tables("rc_ypddnr").Rows(i).Item("zl")
            ...
            Else
                ...
                rcOleDbCommand.Transaction = rcOleDbTrans
                Try
                    rcOleDbCommand.CommandText = "UPDATE oe_ypdd SET fhrq = null,zl = 0 WHERE djh = ? and xh = ?"
                    rcOleDbCommand.Parameters.Clear()
                    rcOleDbCommand.Parameters.Add("@djh", OleDbType.VarChar, 15).Value = rcDataSet.Tables("rc_ypddnr").Rows(i).Item("djh")
                    rcOleDbCommand.Parameters.Add("@xh", OleDbType.Integer, 4).Value = rcDataSet.Tables("rc_ypddnr").Rows(i).Item("xh")
                    rcOleDbCommand.ExecuteNonQuery()
