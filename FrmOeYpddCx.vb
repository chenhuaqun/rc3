...
#Region "确定事件"
    ...
    Private Sub BtnOk_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnOk.Click
        ...
        If rcDataset.Tables("rc_bmxx").Rows.Count <= 0 Then
            ...
            Return
        End If
        Dim strExpBmdm As String = " 部门编码 IS NULL"
        Dim j As Integer
        ...
        Try
            If Me.RadioBtnQdrq.Checked Then
                rcOleDbCommand.CommandText = "Select * From view_oe_ypdd WHERE 0 = 0" & IIf(Trim(TxtCpdm.Text).Length > 0, " AND 产品编码 LIKE '" & Trim(TxtCpdm.Text) & "%'", "") & IIf(TxtKhlh.TextLength > 0, " and 客户料号 LIKE '" & Trim(TxtKhlh.Text) & "%'", "") & IIf(TxtKhcz.TextLength > 0, " and 客户材质 LIKE '" & Trim(TxtKhcz.Text) & "%'", "") & IIf(TxtZydm.TextLength > 0, " and 内销职员编码 LIKE '" & TxtZydm.Text & "%'", "") & IIf(TxtHth.TextLength > 0, " and 合同编码 LIKE '" & TxtHth.Text & "%'", "") & IIf(Trim(TxtKhdm.Text).Length > 0, " and 客户编码 LIKE '" & LTrim(TxtKhdm.Text) & "%'", "") & IIf(Trim(TxtBmdm.Text).Length > 0, " and 部门编码 LIKE '" & TxtBmdm.Text & "%'", "") & IIf(Me.ChbHx.Checked, "", " AND 订单数量 <> 核销数量") & IIf(Me.ChbShdh.Checked, " AND NOT 送货单号 IS NULL", "") & IIf(Trim(TxtCpmc.Text).Length > 0, " AND 产品名称 LIKE '" & TxtCpmc.Text & "%'", "") & IIf(Trim(TxtCpgg.Text).Length > 0, " and 型号规格 LIKE '" & Trim(TxtCpgg.Text) & "%'", "") & IIf(Trim(Me.TxtCpmemo.Text).Length > 0, " and 产品属性 LIKE '" & Trim(Me.TxtCpmemo.Text) & "%'", "") & " AND SUBSTR(单据号,11,5) >= ?  AND SUBSTR(单据号,11,5) <= ? AND (" & strExpBmdm & ") AND 签单日期 <= ? AND 签单日期 >= ?"
            Else
                If Me.RadioBtnScjhrq.Checked Then
                    rcOleDbCommand.CommandText = "Select * From view_oe_ypdd WHERE 0 = 0" & IIf(Trim(TxtCpdm.Text).Length > 0, " AND 产品编码 LIKE '" & Trim(TxtCpdm.Text) & "%'", "") & IIf(TxtKhlh.TextLength > 0, " and 客户料号 LIKE '" & Trim(TxtKhlh.Text) & "%'", "") & IIf(TxtKhcz.TextLength > 0, " and 客户材质 LIKE '" & Trim(TxtKhcz.Text) & "%'", "") & IIf(TxtZydm.TextLength > 0, " and 内销职员编码 LIKE '" & TxtZydm.Text & "%'", "") & IIf(TxtHth.TextLength > 0, " and 合同编码 LIKE '" & TxtHth.Text & "%'", "") & IIf(Trim(TxtKhdm.Text).Length > 0, " and 客户编码 LIKE '" & LTrim(TxtKhdm.Text) & "%'", "") & IIf(Trim(TxtBmdm.Text).Length > 0, " and 部门编码 LIKE '" & TxtBmdm.Text & "%'", "") & IIf(Me.ChbHx.Checked, "", " AND 订单数量 <> 核销数量") & IIf(Me.ChbShdh.Checked, " AND NOT 送货单号 IS NULL", "") & IIf(Trim(TxtCpmc.Text).Length > 0, " AND 产品名称 LIKE '" & TxtCpmc.Text & "%'", "") & IIf(Trim(TxtCpgg.Text).Length > 0, " and 型号规格 LIKE '" & Trim(TxtCpgg.Text) & "%'", "") & IIf(Trim(Me.TxtCpmemo.Text).Length > 0, " and 产品属性 LIKE '" & Trim(Me.TxtCpmemo.Text) & "%'", "") & " AND SUBSTR(单据号,11,5) >= ?  AND SUBSTR(单据号,11,5) <= ? AND (" & strExpBmdm & ") AND 生产交期 <= ? AND 生产交期 >= ?"
                Else
                    rcOleDbCommand.CommandText = "Select * From view_oe_ypdd WHERE 0 = 0" & IIf(Trim(TxtCpdm.Text).Length > 0, " AND 产品编码 LIKE '" & Trim(TxtCpdm.Text) & "%'", "") & IIf(TxtKhlh.TextLength > 0, " and 客户料号 LIKE '" & Trim(TxtKhlh.Text) & "%'", "") & IIf(TxtKhcz.TextLength > 0, " and 客户材质 LIKE '" & Trim(TxtKhcz.Text) & "%'", "") & IIf(TxtZydm.TextLength > 0, " and 内销职员编码 LIKE '" & TxtZydm.Text & "%'", "") & IIf(TxtHth.TextLength > 0, " and 合同编码 LIKE '" & TxtHth.Text & "%'", "") & IIf(Trim(TxtKhdm.Text).Length > 0, " and 客户编码 LIKE '" & LTrim(TxtKhdm.Text) & "%'", "") & IIf(Trim(TxtBmdm.Text).Length > 0, " and 部门编码 LIKE '" & TxtBmdm.Text & "%'", "") & IIf(Me.ChbHx.Checked, "", " AND 订单数量 <> 核销数量") & IIf(Me.ChbShdh.Checked, " AND NOT 送货单号 IS NULL", "") & IIf(Trim(TxtCpmc.Text).Length > 0, " AND 产品名称 LIKE '" & TxtCpmc.Text & "%'", "") & IIf(Trim(TxtCpgg.Text).Length > 0, " and 型号规格 LIKE '" & Trim(TxtCpgg.Text) & "%'", "") & IIf(Trim(Me.TxtCpmemo.Text).Length > 0, " and 产品属性 LIKE '" & Trim(Me.TxtCpmemo.Text) & "%'", "") & " AND SUBSTR(单据号,11,5) >= ?  AND SUBSTR(单据号,11,5) <= ? AND (" & strExpBmdm & ") AND 发货日期 <= ? AND 发货日期 >= ?"
                End If
            End If
            rcOleDbCommand.Parameters.Clear()
        ...
        With rcFrm
            ...
            .ParaDataView = New DataView(rcDataset.Tables("ddlb"), "TRUE", "单据号,行号", DataViewRowState.CurrentRows)
