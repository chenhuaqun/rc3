...
Public Class FrmPmBmRkHzb
    ...
    '建立Datatable
    ReadOnly dtPmBmRkHzb As New DataTable("pmbmrkhzb")

    Private Sub FrmPmBmRkHzb_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        '默认值
        DtpHzrqBegin.Value = getInvBegin(Mid(g_Kjqj, 1, 4), Mid(g_Kjqj, 5, 2))
        DtpHzrqEnd.Value = getInvEnd(Mid(g_Kjqj, 1, 4), Mid(g_Kjqj, 5, 2))
        '创建datatable
        dtPmBmRkHzb.Columns.Add("bmdm", Type.GetType("System.String"))
        dtPmBmRkHzb.Columns.Add("bmmc", Type.GetType("System.String"))
        dtPmBmRkHzb.Columns.Add("sl", Type.GetType("System.Double"))
        dtPmBmRkHzb.Columns.Add("je", Type.GetType("System.Double"))
        rcDataset.Tables.Add(dtPmBmRkHzb)
        With dtPmBmRkHzb
...
#End Region
    ...
    Private Sub BtnOk_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnOk.Click
        ...
        Try
            ...
            rcOleDbCommand.CommandTimeout = 300
            rcOleDbCommand.CommandType = CommandType.Text
            rcOleDbCommand.CommandText = "SELECT aa.bmdm AS bmdm,rc_bmxx.bmmc AS bmmc,aa.sl,aa.je FROM (SELECT inv_rkd.bmdm,SUM(inv_rkd.sl) AS sl,SUM(inv_rkd.je) AS je FROM inv_rkd WHERE inv_rkd.bdelete = 0 AND rkrq >= ? AND rkrq >= ? AND rkrq <= ?" & IIf(Not String.IsNullOrEmpty(Me.TxtCkdm.Text), " AND inv_rkd.ckdm = '" & Me.TxtCkdm.Text & "'", "") & " GROUP BY inv_rkd.bmdm) aa LEFT JOIN rc_bmxx ON rc_bmxx.bmdm = aa.bmdm"
            rcOleDbCommand.Parameters.Clear()
            ...
            rcOleDbDataAdpt.SelectCommand = rcOleDbCommand
            If rcDataset.Tables("pmbmrkhzb") IsNot Nothing Then
                rcDataset.Tables("pmbmrkhzb").Clear()
            End If
            rcOleDbDataAdpt.Fill(rcDataset, "pmbmrkhzb")
        Catch ex As Exception
            MsgBox("程序错误。" & Chr(13) & ex.Message, MsgBoxStyle.OkOnly + MsgBoxStyle.Question, "提示信息")
            Return
        Finally
        ...
        rcDataRow = rcDataset.Tables("pmbmrkhzb").NewRow
        rcDataRow.Item("bmdm") = "合计"
        rcDataRow.Item("sl") = dtPmBmRkHzb.Compute("Sum(sl)", "")
        rcDataRow.Item("je") = dtPmBmRkHzb.Compute("Sum(je)", "")
        rcDataset.Tables("pmbmrkhzb").Rows.Add(rcDataRow)

        '调用表单
        Dim rcFrm As New FrmPmBmRkHzbz
        With rcFrm
            .ParaDataSet = rcDataset
            .ParaDataView = New DataView(rcDataset.Tables("pmbmrkhzb"), "TRUE", "bmdm", DataViewRowState.CurrentRows)
