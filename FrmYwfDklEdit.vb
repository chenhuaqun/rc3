...
#Region "初始化"
    ...
    Private Sub FrmYwfDklEdit_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Me.TxtXh.DataBindings.Add("Text", rcDataView, "xh")
        Me.TxtMc.DataBindings.Add("Text", rcDataView, "mc")
        Me.TxtDkbl.DataBindings.Add("Text", rcDataView, "dkbl")
...
#Region "控键回车键的处理"
    ...
    Private Sub Control_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles TxtXh.KeyPress, TxtDkbl.KeyPress, TxtMc.KeyPress
...
#Region "设置控件"
    ...
    Private Sub SetAll(ByVal medit As Boolean)
        ...
        Else
            Me.TxtXh.Enabled = True
            Me.TxtMc.Enabled = True
            Me.TxtDkbl.Enabled = True
...
#Region "保存"
    ...
    Private Sub SaveEvent()
        '保存
        If isAdding Then
            If Trim(TxtXh.Text).Length = 0 Then
                Return
            End If
            'REM 增加保存
            Try
                ...
                rcOleDbCommand.Parameters.Add("@xh", OleDbType.VarChar, 6).Value = Trim(Me.TxtXh.Text)
                rcOleDbCommand.Parameters.Add("@mc", OleDbType.VarChar, 30).Value = Trim(Me.TxtMc.Text)
                rcOleDbCommand.Parameters.Add("@dkbl", OleDbType.Numeric, 6).Value = Me.TxtDkbl.Text
        ...
        Else
            ...
            Try
                ...
                rcOleDbCommand.CommandTimeout = 300
                rcOleDbCommand.CommandType = CommandType.Text
                rcOleDbCommand.CommandText = "UPDATE gl_ywfdkl SET mc = ? , dkbl = ?  WHERE  xh = ?"
                rcOleDbCommand.Parameters.Clear()
                rcOleDbCommand.Parameters.Add("@mc", OleDbType.VarChar, 30).Value = Trim(TxtMc.Text)
                rcOleDbCommand.Parameters.Add("@dkbl", OleDbType.Numeric, 6).Value = Me.TxtDkbl.Text
                rcOleDbCommand.Parameters.Add("@xh", OleDbType.VarChar, 4).Value = Trim(TxtXh.Text)
                rcOleDbCommand.ExecuteNonQuery()
                '填充数据
                rcOleDbCommand.CommandText = "SELECT xh,mc,dkbl FROM gl_ywfdkl ORDER BY xh"
                rcOleDbCommand.Parameters.Clear()
