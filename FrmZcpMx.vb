...
Public Class FrmZcpMx
    ...
    Private Sub FrmZcpMx_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        '默认值
...
#End Region
    ...
    Private Sub BtnOk_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnOk.Click
        Try
            ...
            rcOleDbCommand.CommandType = CommandType.Text
            If bCostRegion Then
                rcOleDbCommand.CommandText = "SELECT pm_zcp.bmdm,rc_costregion.crmc AS bmmc,pm_zcp.cpdm,rc_cpxx.cpmc,rc_cpxx.dw,pm_zcp.gxdm,rc_gxxx.gxmc,pm_zcp.zcpsl,pm_zcp.zcpje,pm_zcp.clcb,pm_zcp.rgcb,pm_zcp.nycb,pm_zcp.zjcb,pm_zcp.glcb FROM pm_zcp LEFT JOIN rc_cpxx ON pm_zcp.cpdm = rc_cpxx.cpdm LEFT JOIN rc_gxxx ON pm_zcp.gxdm = rc_gxxx.gxdm LEFT JOIN rc_costregion ON rc_costregion.crdm = pm_zcp.bmdm WHERE cperiod = ?" & IIf(Not String.IsNullOrEmpty(Me.TxtBmdm.Text), " AND pm_zcp.bmdm ='" & Me.TxtBmdm.Text & "'", "") & " ORDER BY pm_zcp.bmdm,pm_zcp.cpdm,pm_zcp.gxdm"
            Else
                rcOleDbCommand.CommandText = "SELECT pm_zcp.bmdm,rc_bmxx.bmmc,pm_zcp.cpdm,rc_cpxx.cpmc,rc_cpxx.dw,pm_zcp.gxdm,rc_gxxx.gxmc,pm_zcp.zcpsl,pm_zcp.zcpje,pm_zcp.clcb,pm_zcp.rgcb,pm_zcp.nycb,pm_zcp.zjcb,pm_zcp.glcb FROM pm_zcp LEFT JOIN rc_cpxx ON pm_zcp.cpdm = rc_cpxx.cpdm LEFT JOIN rc_gxxx ON pm_zcp.gxdm = rc_gxxx.gxdm LEFT JOIN rc_bmxx ON rc_bmxx.bmdm = pm_zcp.bmdm WHERE cperiod = ?" & IIf(Not String.IsNullOrEmpty(Me.TxtBmdm.Text), " AND pm_zcp.bmdm ='" & Me.TxtBmdm.Text & "'", "") & " ORDER BY pm_zcp.bmdm,pm_zcp.cpdm,pm_zcp.gxdm"
            End If
            rcOleDbCommand.Parameters.Clear()
        ...
        If rcDataset.Tables("pm_zcp").Rows.Count > 0 Then
            ...
            rcDataRow.Item("clcb") = rcDataset.Tables("pm_zcp").Compute("Sum(clcb)", "")
            rcDataRow.Item("rgcb") = rcDataset.Tables("pm_zcp").Compute("Sum(rgcb)", "")
            rcDataRow.Item("nycb") = rcDataset.Tables("pm_zcp").Compute("Sum(nycb)", "")
            rcDataRow.Item("zjcb") = rcDataset.Tables("pm_zcp").Compute("Sum(zjcb)", "")
            rcDataRow.Item("glcb") = rcDataset.Tables("pm_zcp").Compute("Sum(glcb)", "")
            rcDataset.Tables("pm_zcp").Rows.Add(rcDataRow)

        End If
        '调用表单
        Dim rcFrm As New FrmZcpMxz
        With rcFrm
            '.ParaDataSet = rcDataset
            .ParaDataView = New DataView(rcDataset.Tables("pm_zcp"), "TRUE", "bmdm,cpdm,gxdm", DataViewRowState.CurrentRows)
