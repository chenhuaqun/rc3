...
Module MdlReadKcdj
    ...
    Friend Function ReadKcdj(ByVal strYear As String, ByVal strCpdm As String, ByVal strCkdm As String, ByVal strDjh As String) As Double
        '建立数据适配器
        ...
        Try
           ...
           " (SELECT inv_cpyeb.cpdm,sum(qcsl) as qcsl,sum(qcje) as qcje FROM inv_cpyeb WHERE kjnd = ?" & IIf(Not String.IsNullOrEmpty(strCkdm), " AND inv_cpyeb.ckdm ='" & strCkdm & "'", "") & " GROUP BY inv_cpyeb.cpdm) cpnc" & _
           ...
           " Left join (SELECT po_rkd.cpdm,sum(po_rkd.sl) as qccgrksl,sum(po_rkd.je) as qccgrkje FROM po_rkd WHERE po_rkd.bdelete = 0 AND po_rkd.rkrq >= ? and po_rkd.rkrq >= ? and po_rkd.rkrq < ? AND po_rkd.djh <> '" & strDjh & "'" & IIf(Not String.IsNullOrEmpty(strCkdm), " AND po_rkd.ckdm ='" & strCkdm & "'", "") & " GROUP BY po_rkd.cpdm) qccgrk ON cpnc.cpdm = qccgrk.cpdm" & _
        ...
        If rcDataSet.Tables("t_cpyeb").Rows.Count > 0 Then
            If rcDataSet.Tables("t_cpyeb").Rows(0).Item("kcdj") = 0 Then
                Try
                    rcOleDbConn.Open()
                    rcOleDbCommand.Connection = rcOleDbConn
                    rcOleDbCommand.CommandTimeout = 300
                    rcOleDbCommand.CommandType = CommandType.Text
                    rcOleDbCommand.CommandText = "SELECT COALESCE(bzcb,0.0) AS kcdj FROM rc_cpxx WHERE cpdm = ?"
                    rcOleDbCommand.Parameters.Clear()
