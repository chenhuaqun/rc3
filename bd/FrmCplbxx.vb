...
#Region "定义变量"
    ...
    ReadOnly dtCplb As New DataTable("rc_cplb")
    '打印文档
...
#Region "初始化"
    ...
    Private Sub FrmCplbxx_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load

        '数据绑定
        dtCplb.Columns.Add("lbdm", Type.GetType("System.String"))
        dtCplb.Columns.Add("lbmc", Type.GetType("System.String"))
        dtCplb.Columns.Add("lbsm", Type.GetType("System.String"))
        ...
        Finally
            rcOleDbConn.Close()
        End Try
        rcDataView = New DataView(rcDataset.Tables("rc_cplb"))
...
#Region "打印、打印预览"
    ...
    Private Sub PreparePrintData()
        'Dim rft1 As String =Application.StartupPath + "\reports\cplbxx.csv"
        Dim rft As String =Application.StartupPath + "\reports\cplbxx.rft"
        ...
        For i = 0 To rcDataset.Tables("rc_cplb").Rows.Count - 1
            rcRps.Text(i + 1, 1) = Trim(rcDataset.Tables("rc_cplb").Rows(i).Item("lbdm"))
            rcRps.Text(i + 1, 2) = Trim(rcDataset.Tables("rc_cplb").Rows(i).Item("lbmc"))
            rcRps.Text(i + 1, 3) = Trim(rcDataset.Tables("rc_cplb").Rows(i).Item("lbsm"))
        Next
        '取RPS数据
        Try
            rcOleDbConn.Open()
            ...
            rcOleDbCommand.CommandTimeout = 300
            rcOleDbCommand.CommandType = CommandType.Text
            rcOleDbCommand.CommandText = "SELECT * FROM rc_rps WHERE rpsid = 'CPLBXX'"
            rcOleDbCommand.Parameters.Clear()
            rcOleDbDataAdpt.SelectCommand = rcOleDbCommand
...
#Region "输出"
    ...
    Private Sub BtnExport_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnExport.Click, MnuiExport.Click
        '导出数据
        Exports2Excel(rcDataset.Tables("rc_cplb"))
    End Sub
    ...
    Private Sub BtnDelete_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnDelete.Click, MnuiDelete.Click
        ...
        If MessageBox.Show("您真地要删除吗？" & Trim(BindingContext(rcDataView, "").Current("lbdm")), "提示信息", MessageBoxButtons.OKCancel, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) = DialogResult.OK Then
            ...
            Try
                ...
                rcOleDbCommand.CommandTimeout = 300
                rcOleDbCommand.CommandType = CommandType.Text
                rcOleDbCommand.CommandText = "DELETE FROM rc_cplb WHERE lbdm = ?"
                rcOleDbCommand.Parameters.Clear()
