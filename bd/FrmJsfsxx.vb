...
#Region "定义变量"
    ...
    ReadOnly dtJsfs As New DataTable("rc_jsfs")
    '打印文档
...
#Region "初始化"
    ...
    Private Sub FrmJsfsxx_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load

        '数据绑定
        dtJsfs.Columns.Add("jsfsdm", Type.GetType("System.String"))
        ...
        Finally
            rcOleDbConn.Close()
        End Try
        rcDataView = New DataView(rcDataset.Tables("rc_jsfs"))
...
#Region "打印、打印预览"
    ...
    Private Sub PreparePrintData()
        'Dim rft1 As String =Application.StartupPath + "\reports\jsfsxx.csv"
        Dim rft As String =Application.StartupPath + "\reports\jsfsxx.rft"
        ...
        Dim i As Integer
        For i = 0 To rcDataset.Tables("rc_jsfs").Rows.Count - 1
            rcRps.Text(i + 1, 1) = Trim(rcDataset.Tables("rc_jsfs").Rows(i).Item("jsfsdm"))
            rcRps.Text(i + 1, 2) = Trim(rcDataset.Tables("rc_jsfs").Rows(i).Item("jsfsmc"))
            rcRps.Text(i + 1, 3) = Trim(rcDataset.Tables("rc_jsfs").Rows(i).Item("jsfssm"))
        Next
        '取RPS数据
        Try
            rcOleDbConn.Open()
            ...
            rcOleDbCommand.CommandTimeout = 300
            rcOleDbCommand.CommandType = CommandType.Text
            rcOleDbCommand.CommandText = "SELECT * FROM rc_rps WHERE rpsid = 'JSFS'"
            rcOleDbCommand.Parameters.Clear()
            rcOleDbDataAdpt.SelectCommand = rcOleDbCommand
...
#Region "输出"
    ...
    Private Sub BtnExport_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnExport.Click, MnuiExport.Click
        '导出数据
        Exports2Excel(rcDataset.Tables("rc_jsfs"))
    End Sub
...
#Region "删除"
    ...
    Private Sub BtnDelete_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnDelete.Click, MnuiDelete.Click
        '删除
        '删除数据
        If MessageBox.Show("您真地要删除吗？" & Trim(BindingContext(rcDataView, "").Current("jsfsdm")), "提示信息", MessageBoxButtons.OKCancel, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) = DialogResult.OK Then
            currentPos = BindingContext(rcDataView, "").Position
            If Trim(BindingContext(rcDataView, "").Current("jsfsdm")) = "" Then
            ...
            Try
                ...
                rcOleDbCommand.CommandTimeout = 300
                rcOleDbCommand.CommandType = CommandType.Text
                rcOleDbCommand.CommandText = "DELETE FROM rc_jsfs WHERE jsfsdm = ?"
                rcOleDbCommand.Parameters.Clear()
                rcOleDbCommand.Parameters.AddWithValue("@jsfsdm", BindingContext(rcDataView, "").Current("jsfsdm"))
