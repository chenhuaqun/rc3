...
Public Class FrmKczlFd
    ...
    ReadOnly dtKczlfd As New DataTable("rc_kczlfd")
    '打印文档
    ...
    Private Sub FrmKczlFd_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            rcOleDbConn.Open()
        ...
        Finally
            rcOleDbConn.Close()
        End Try
        rcDataView = New DataView(rcDataset.Tables("rc_kczlfd"))
...
#Region "打印、打印预览"
    ...
    Private Sub PreparePrintData()
        'Dim rft1 As String =Application.StartupPath + "\reports\gxxx.csv"
        Dim rft As String =Application.StartupPath + "\reports\gxxx.rft"
        ...
        For i = 0 To rcDataset.Tables("rc_kczlfd").Rows.Count - 1
            rcRps.Text(i + 1, 1) = Trim(rcDataset.Tables("rc_kczlfd").Rows(i).Item("fdxh"))
            rcRps.Text(i + 1, 2) = Trim(rcDataset.Tables("rc_kczlfd").Rows(i).Item("zhangling"))
            rcRps.Text(i + 1, 3) = Trim(rcDataset.Tables("rc_kczlfd").Rows(i).Item("jitibilv"))
        Next
        '取RPS数据
        Try
            rcOleDbConn.Open()
...
#Region "输出"
    ...
    Private Sub BtnExport_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnExport.Click, MnuiExport.Click
        '导出数据
        Exports2Excel(rcDataset.Tables("rc_kczlfd"))
    End Sub
...
#Region "新增"
    ...
    Private Sub BtnNew_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnNew.Click, MnuiNew.Click
        '新增
        Dim rcFrm As New FrmKczlFdEdit
        With rcFrm
            .ParaAdding = True
...
#Region "删除"
    ...
    Private Sub BtnDelete_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnDelete.Click, MnuiDelete.Click
        '删除
        '删除数据
        If MessageBox.Show("您真地要删除吗？" & Trim(BindingContext(rcDataView, "").Current("fdxh")), "提示信息", MessageBoxButtons.OKCancel, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) = DialogResult.OK Then
            currentPos = BindingContext(rcDataView, "").Position
            If Trim(BindingContext(rcDataView, "").Current("fdxh")) = "" Then
            ...
            Try
                ...
                rcOleDbCommand.CommandTimeout = 300
                rcOleDbCommand.CommandType = CommandType.Text
                rcOleDbCommand.CommandText = "DELETE FROM rc_kczlfd WHERE fdxh = ? AND fadm = ?"
                rcOleDbCommand.Parameters.Clear()
                rcOleDbCommand.Parameters.AddWithValue("@fdxh", BindingContext(rcDataView, "").Current("fdxh"))
                rcOleDbCommand.Parameters.Add("@fadm", OleDbType.VarChar, 12).Value = strFadm
