...
#Region "保存事件"
    ...
    Private Sub TsbSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TsbSave.Click
        ...
        '    End If
        '    If rcDataset.Tables("result").Rows(i).Item("在产材料数量").GetType.ToString = "System.DBNull" Then
        '        rcDataset.Tables("result").Rows(i).Item("在产材料数量") = 0.0
        '    End If
        'Next
        Try
            rcOleDbConn.Open()
            ...
            If Me.RadioButton1.Checked Then
                '删除已保存的数据
                rcOleDbCommand.CommandText = "DELETE FROM pm_zccl WHERE cperiod = ?"
            ...
            For i = 0 To rcDataset.Tables("result").Rows.Count - 1
                ...
                If rcDataset.Tables("pm_zccl").Rows.Count = 0 Then
                    ...
                    rcOleDbCommand.Parameters.Add("@cpdm", OleDbType.VarChar, 15).Value = Trim(rcDataset.Tables("result").Rows(i).Item("物料编码")).ToUpper
                    rcOleDbCommand.Parameters.Add("@gxdm", OleDbType.VarChar, 12).Value = Trim(rcDataset.Tables("result").Rows(i).Item("工序编码")).ToUpper
                    rcOleDbCommand.Parameters.Add("@zcclsl", OleDbType.Numeric, 18).Value = rcDataset.Tables("result").Rows(i).Item("在产材料数量")
                    rcOleDbCommand.ExecuteNonQuery()
                Else
                    rcOleDbCommand.CommandText = "UPDATE pm_zccl SET zcclsl = zcclsl + ? WHERE cperiod = ? AND bmdm = ? AND cpdm = ? AND gxdm = ?"
                    rcOleDbCommand.Parameters.Clear()
                    rcOleDbCommand.Parameters.Add("@zcclsl", OleDbType.Numeric, 18).Value = rcDataset.Tables("result").Rows(i).Item("在产材料数量")
                    rcOleDbCommand.Parameters.Add("@cperiod", OleDbType.VarChar, 6).Value = strKjqj
                    rcOleDbCommand.Parameters.Add("@bmdm", OleDbType.VarChar, 12).Value = Trim(rcDataset.Tables("result").Rows(i).Item("部门编码")).ToUpper
        ...
        Finally
            rcOleDbConn.Close()
        End Try
        MsgBox("期末在产材料数量读入完成,请检查数据。", MsgBoxStyle.OkOnly + MsgBoxStyle.Question, "提示信息")
        Me.Close()
    End Sub
