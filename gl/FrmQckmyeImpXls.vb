    ...
    Private Sub TsbSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TsbSave.Click
        ...
        Try
            ...
            rcOleDbCommand.CommandText = "DELETE FROM gl_kmyeb WHERE kjnd = ?"
                ...
                rcOleDbCommand.Parameters.Add("@cperiod", OleDbType.VarChar, 4).Value = Mid(g_Kjqj, 1, 4)
            ...
            For i = 0 To rcDataset.Tables("result").Rows.Count - 1
                ...
                rcOleDbCommand.Parameters.Add("@kmdm", OleDbType.VarChar, 15).Value = Trim(rcDataset.Tables("result").Rows(i).Item("科目编码")).ToUpper
                If rcDataset.Tables("result").Rows(i).Item("币种编码").GetType.ToString <> "System.DBNull" Then
                    rcOleDbCommand.Parameters.Add("@wbdm", OleDbType.VarChar, 4).Value = rcDataset.Tables("result").Rows(i).Item("币种编码")
                Else
                    rcOleDbCommand.Parameters.Add("@wbdm", OleDbType.VarChar, 4).Value = "CNY"
                End If
                ...
                Else
                    rcOleDbCommand.Parameters.Add("@bmdm", OleDbType.VarChar, 12).Value = "~"
                End If
                ...
                rcOleDbCommand.Parameters.Add("@khdm", OleDbType.VarChar, 15).Value = IIf(Mid(rcDataset.Tables("result").Rows(i).Item("科目编码"), 1, 4) = "1122", rcDataset.Tables("result").Rows(i).Item("往来单位编码"), "~")
                ...
                rcOleDbCommand.Parameters.Add("@jd", OleDbType.VarChar, 2).Value = rcDataset.Tables("result").Rows(i).Item("余额方向")
                rcOleDbCommand.Parameters.Add("@ncsl", OleDbType.Numeric, 18).Value = 0.0
                rcOleDbCommand.Parameters.Add("@ncwb", OleDbType.Numeric, 18).Value = rcDataset.Tables("result").Rows(i).Item("期初余额金额")
                rcOleDbCommand.Parameters.Add("@ncje", OleDbType.Numeric, 14).Value = rcDataset.Tables("result").Rows(i).Item("期初余额金额")
                rcOleDbCommand.ExecuteNonQuery()
            Next
            rcOleDbTrans.Commit()
        Catch ex As Exception
        ...
        Finally
            rcOleDbConn.Close()
        End Try
        MsgBox("总账期初余额读入完成,请检查数据。", MsgBoxStyle.OkOnly + MsgBoxStyle.Question, "提示信息")
        Me.Close()
    End Sub
