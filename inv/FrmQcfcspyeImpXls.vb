...
Public Class FrmQcfcspyeImpXls
    '建立数据适配器
...
#Region "保存事件"
    ...
    Private Sub TsbSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TsbSave.Click
        ...
        Try
            ...
            '覆盖
            rcOleDbCommand.CommandText = "DELETE FROM inv_fcspyeb WHERE kjnd = ? AND (qcsl <> 0 or qcfzsl <>0 or qcje <> 0)"
            rcOleDbCommand.Parameters.Clear()
            rcOleDbCommand.Parameters.Add("@kjnd", OleDbType.VarChar, 6).Value = Mid(g_Kjqj, 1, 4)
            rcOleDbCommand.ExecuteNonQuery()

            For i = 0 To rcDataset.Tables("result").Rows.Count - 1
                rcOleDbCommand.CommandText = "SELECT cpdm FROM inv_fcspyeb WHERE kjnd = ? AND cpdm = ? AND khdm = ? AND bmdm = ?"
                rcOleDbCommand.Parameters.Clear()
                rcOleDbCommand.Parameters.Add("@kjnd", OleDbType.VarChar, 6).Value = Mid(g_Kjqj, 1, 4)
                rcOleDbCommand.Parameters.Add("@cpdm", OleDbType.VarChar, 15).Value = Trim(rcDataset.Tables("result").Rows(i).Item("物料编码")).ToUpper
                rcOleDbCommand.Parameters.Add("@khdm", OleDbType.VarChar, 12).Value = Trim(rcDataset.Tables("result").Rows(i).Item("客户编码")).ToUpper
                ...
                If rcDataset.Tables("inv_fcspyeb") IsNot Nothing Then
                    rcDataset.Tables("inv_fcspyeb").Clear()
                End If
                rcOleDbDataAdpt.Fill(rcDataset, "inv_fcspyeb")
                If rcDataset.Tables("inv_fcspyeb").Rows.Count > 0 Then
                    rcOleDbCommand.CommandText = "UPDATE inv_fcspyeb SET qcsl = qcsl + ?,qcfzsl = qcfzsl + ?,qcje = qcje + ? WHERE kjnd = ? AND cpdm = ? AND khdm = ? AND bmdm = ?"
                    rcOleDbCommand.Parameters.Clear()
                    rcOleDbCommand.Parameters.Add("@qcsl", OleDbType.Numeric, 18).Value = rcDataset.Tables("result").Rows(i).Item("期初发出商品数量")
                    rcOleDbCommand.Parameters.Add("@qcfzsl", OleDbType.Numeric, 18).Value = rcDataset.Tables("result").Rows(i).Item("期初发出商品辅数量")
                    rcOleDbCommand.Parameters.Add("@qcje", OleDbType.Numeric, 14).Value = rcDataset.Tables("result").Rows(i).Item("期初发出商品金额")
                    rcOleDbCommand.Parameters.Add("@kjnd", OleDbType.VarChar, 6).Value = Mid(g_Kjqj, 1, 4)
                    rcOleDbCommand.Parameters.Add("@cpdm", OleDbType.VarChar, 15).Value = Trim(rcDataset.Tables("result").Rows(i).Item("物料编码")).ToUpper
                    rcOleDbCommand.Parameters.Add("@khdm", OleDbType.VarChar, 12).Value = Trim(rcDataset.Tables("result").Rows(i).Item("客户编码")).ToUpper
                    rcOleDbCommand.Parameters.Add("@bmdm", OleDbType.VarChar, 12).Value = Trim(rcDataset.Tables("result").Rows(i).Item("部门编码")).ToUpper
                    rcOleDbCommand.ExecuteNonQuery()
                Else
                    rcOleDbCommand.CommandText = "INSERT INTO inv_fcspyeb (kjnd,cpdm,khdm,bmdm,qcsl,qcfzsl,qcje,idsl,idfzsl,idje) values (?,?,?,?,?,?,?,0.0,0.0,0.0)"
                    rcOleDbCommand.Parameters.Clear()
                    rcOleDbCommand.Parameters.Add("@kjnd", OleDbType.VarChar, 6).Value = Mid(g_Kjqj, 1, 4)
                    rcOleDbCommand.Parameters.Add("@cpdm", OleDbType.VarChar, 15).Value = Trim(rcDataset.Tables("result").Rows(i).Item("物料编码")).ToUpper
                    rcOleDbCommand.Parameters.Add("@khdm", OleDbType.VarChar, 12).Value = Trim(rcDataset.Tables("result").Rows(i).Item("客户编码")).ToUpper
        ...
        Finally
            rcOleDbConn.Close()
        End Try
        MsgBox("期初发出商品余额读入完成,请检查数据。", MsgBoxStyle.OkOnly + MsgBoxStyle.Question, "提示信息")
        Me.Close()
    End Sub
