...
Public Class FrmOeFhdSh
    ...
    Private Sub FrmOeFhdSh_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        '默认值
    ...
    Private Sub BtnOk_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnOk.Click
        ...
        Try
            ...
            rcOleDbCommand.CommandTimeout = 300
            rcOleDbCommand.CommandType = CommandType.Text
            rcOleDbCommand.CommandText = "SELECT oe_fhd.djh FROM oe_fhd,rc_lx WHERE SUBSTR(oe_fhd.djh,1,4) = rc_lx.pzlxdm AND SUBSTR(oe_fhd.djh,5,4) = rc_lx.kjnd AND lxgs = '发货通知书' AND oe_fhd.bdelete = 0 AND oe_fhd.fhrq >= ? AND oe_fhd.fhrq <= ? AND SUBSTR(oe_fhd.djh,11, 5) >= ?  AND SUBSTR(oe_fhd.djh,11, 5) <= ?" & IIf(ChbSh.Checked, " and oe_fhd.shr IS NULL", "") & IIf(Not String.IsNullOrEmpty(Me.TxtHth.Text), " and oe_fhd.hth = '" & Trim(Me.TxtHth.Text) & "'", "") & IIf(Trim(Me.TxtKhdm.Text).Length > 0, " and oe_fhd.khdm = '" & Trim(Me.TxtKhdm.Text) & "'", "") & IIf(Me.CmbPzlxjc.SelectedValue <> "0000", " AND SUBSTR(oe_fhd.djh,1, 4) ='" & Me.CmbPzlxjc.SelectedValue & "'", "") & " GROUP BY oe_fhd.djh ORDER BY oe_fhd.djh"
            rcOleDbCommand.Parameters.Clear()
            rcOleDbCommand.Parameters.Add("@fhrq", OleDbType.Date, 8).Value = DtpBegin.Value
            rcOleDbCommand.Parameters.Add("@fhrq", OleDbType.Date, 8).Value = Me.DtpEnd.Value
