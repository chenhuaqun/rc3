...
Public Class FrmOeFpSh
    ...
    Private Sub FrmOeFpSh_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        '默认值
    ...
    Private Sub BtnOk_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnOk.Click
        ...
        Try
            ...
            rcOleDbCommand.CommandTimeout = 300
            rcOleDbCommand.CommandType = CommandType.Text
            rcOleDbCommand.CommandText = "SELECT oe_fp.djh FROM oe_fp,rc_lx WHERE SUBSTR(oe_fp.djh,1,4) = rc_lx.pzlxdm AND SUBSTR(oe_fp.djh,5,4) = rc_lx.kjnd AND lxgs = '产品销售单' AND oe_fp.bdelete = 0 AND oe_fp.fprq >= ? AND oe_fp.fprq <= ? AND SUBSTR(oe_fp.djh,11, 5) >= ?  AND SUBSTR(oe_fp.djh,11, 5) <= ?" & IIf(ChbSh.Checked, " and oe_fp.shr IS NULL", "") & IIf(Not String.IsNullOrEmpty(Me.TxtHth.Text), " and oe_fp.hth = '" & Trim(Me.TxtHth.Text) & "'", "") & IIf(Trim(Me.TxtKhdm.Text).Length > 0, " and oe_fp.khdm = '" & Trim(Me.TxtKhdm.Text) & "'", "") & IIf(Me.CmbPzlxjc.SelectedValue <> "0000", " AND SUBSTR(oe_fp.djh,1, 4) ='" & Me.CmbPzlxjc.SelectedValue & "'", "") & " GROUP BY oe_fp.djh ORDER BY oe_fp.djh"
            rcOleDbCommand.Parameters.Clear()
