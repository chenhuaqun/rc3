...
#Region "确定事件"
    ...
    Private Sub BtnOk_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnOk.Click
        If Me.ChbLbfs.Checked Then
            ...
            rcDataRow.Item("clcb") = rcDataset.Tables("rkdlb").Compute("Sum(clcb)", "")
            rcDataRow.Item("rgcb") = rcDataset.Tables("rkdlb").Compute("Sum(rgcb)", "")
            rcDataRow.Item("nycb") = rcDataset.Tables("rkdlb").Compute("Sum(nycb)", "")
            rcDataRow.Item("zjcb") = rcDataset.Tables("rkdlb").Compute("Sum(zjcb)", "")
            rcDataRow.Item("glcb") = rcDataset.Tables("rkdlb").Compute("Sum(glcb)", "")
        ...
        Else
            ...
            Try
                ...
                rcOleDbCommand.CommandTimeout = 300
                rcOleDbCommand.CommandType = CommandType.Text
                rcOleDbCommand.CommandText = "SELECT djh FROM inv_rkd,rc_lx WHERE SUBSTR(inv_rkd.djh,1,4) = rc_lx.pzlxdm AND SUBSTR(inv_rkd.djh,5,4) = rc_lx.kjnd AND lxgs = '产品入库单' AND rkrq >= ? AND rkrq <= ? AND SUBSTR(djh,11,5) >= ?  AND SUBSTR(djh,11,5) <= ?" & IIf(Me.CmbPzlxjc.SelectedValue <> "0000", " AND SUBSTR(inv_rkd.djh,1, 4) ='" & Me.CmbPzlxjc.SelectedValue & "'", "") & IIf(Me.TxtZydm.TextLength > 0, " and inv_rkd.zydm = '" & TxtZydm.Text & "'", "") & IIf(TxtBmdm.TextLength > 0, " and inv_rkd.bmdm = '" & TxtBmdm.Text & "'", "") & IIf(TxtCkdm.TextLength > 0, " and inv_rkd.ckdm = '" & TxtCkdm.Text & "'", "") & " GROUP BY inv_rkd.djh ORDER BY inv_rkd.djh"
                rcOleDbCommand.Parameters.Clear()
                rcOleDbCommand.Parameters.Add("@rkrq1", OleDbType.Date, 8).Value = DtpRkrqBegin.Value
                rcOleDbCommand.Parameters.Add("@rkrq2", OleDbType.Date, 8).Value = DtpRkrqEnd.Value
                rcOleDbCommand.Parameters.Add("@djh1", OleDbType.VarChar, 5).Value = NudDjhBegin.Value.ToString.PadLeft(5, "0")
