...
#Region "初始化"
    ...
    Private Sub FrmOeRkdSrz_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        '设置DataGridView
        ...
        Try
            ...
            rcOleDbCommand.CommandText = "SELECT pzlxdm,pzlxjc FROM rc_lx WHERE lxgs = '产品入库单' and kjnd = '" & strYear & "' ORDER BY pzlxdm"
            ...
            'rcOleDbCommand.CommandText = "SELECT zydm,zymc,zysm FROM rc_zyxx ORDER BY zydm"
            'rcOleDbCommand.Parameters.Clear()
            'rcOleDbDataAdpt.SelectCommand = rcOleDbCommand
        ...
        For i = 0 To rcDataset.Tables("rc_lx").Rows.Count - 1
            If rcDataset.Tables("rc_lx").Rows(i).Item("pzlxdm") = "SCRK" Then
                CmbPzlxjc.SelectedValue = "SCRK"
            End If
        Next
...
#Region "控键回车键的处理"
    ...
    Private Sub Control_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles CmbPzlxjc.KeyPress, TxtDjh.KeyPress, DtpRkrq.KeyPress, TxtZydm.KeyPress, TxtBmdm.KeyPress, TxtCkdm.KeyPress
        ...
        If rcDataset.Tables("rc_pzno").Rows.Count > 0 Then
            If rcDataset.Tables("rc_pzno").Rows(0).Item("pzno") >= Mid(Trim(Me.TxtDjh.Text), 15, 5) Then
                ...
                Try
                    ...
                    rcOleDbCommand.CommandTimeout = 300
                    rcOleDbCommand.CommandType = CommandType.Text
                    rcOleDbCommand.CommandText = "SELECT inv_rkd.djh,TRUNC(inv_rkd.rkrq,'mi') AS rkrq,inv_rkd.bdelete,inv_rkd.zydm,inv_rkd.zymc,inv_rkd.bmdm,inv_rkd.bmmc,inv_rkd.ckdm,inv_rkd.ckmc,inv_rkd.bmrp,inv_rkd.srr,inv_rkd.shr,inv_rkd.jzr FROM inv_rkd WHERE (djh = ? )"
                    rcOleDbCommand.Parameters.Clear()
                    ...
                    If Not String.IsNullOrEmpty(e.FormattedValue) Then
                        ...
                        Try
                            ...
                            If rcDataset.Tables("rc_cpxx").Rows.Count > 0 Then
                                ...
                                End If
                                '取最新库存数量
                                rcOleDbCommand.CommandText = "SELECT COALESCE(Sum(qcsl+idsl),0.0) as kcsl,COALESCE(Sum(qcfzsl+idfzsl),0.0) as kcfzsl FROM inv_cpyeb WHERE kjnd = ? AND cpdm = ?"
                                rcOleDbCommand.Parameters.Clear()
                                ...
                                If rcDataset.Tables("inv_cpyeb").Rows.Count = 1 Then
                                    Me.LblMsg.Text = "当前库存数量：" & rcDataset.Tables("inv_cpyeb").Rows(0).Item("kcsl") & "库存辅数量：" & rcDataset.Tables("inv_cpyeb").Rows(0).Item("kcfzsl")
                                Else
                                    Me.LblMsg.Text = "当前库存数量： 0.00 库存辅数量：0.00"
                                End If
                            Else
...
#Region "新单事件"
    ...
    Private Sub ShowNewDjh()
        ...
        Try
            ...
            rcOleDbCommand.CommandTimeout = 300
            rcOleDbCommand.CommandType = CommandType.Text
            rcOleDbCommand.CommandText = "SELECT pzlxdm,pzno" & strMonth & " as pzno FROM rc_lx WHERE pzlxdm = ? and lxgs = '产品入库单' and kjnd = '" & strYear & "'"
            rcOleDbCommand.Parameters.Clear()
    ...
    Private Sub SaveEvent(ByVal blnPrint As Boolean)
            ...
            For i = 0 To rcDataset.Tables("rc_rkdnr").Rows.Count - 1
                If rcDataset.Tables("rc_rkdnr").Rows(i).RowState <> DataRowState.Deleted Then
                    ...
                    If rcDataset.Tables("cpdmcnt").Rows.Count <> 1 Then
                        MsgBox(Trim(rcDataset.Tables("rc_rkdnr").Rows(i).Item("cpdm")) & "产品编码不正确，请检查。", MsgBoxStyle.OkOnly + MsgBoxStyle.Exclamation, "提示信息")
                        Return
                    Else
                        '判断标准成本是不是为零
                        If rcDataset.Tables("cpdmcnt").Rows(0).Item("bzcb") = 0 Then
                            MsgBox(Trim(rcDataset.Tables("rc_rkdnr").Rows(i).Item("cpdm")) & "产品的标准成本为零，不能保存，请检查。", MsgBoxStyle.OkOnly + MsgBoxStyle.Exclamation, "提示信息")
                            Return
                        End If
                    End If
        ...
        Try
            ...
            For i = 0 To rcDataset.Tables("rc_rkdnr").Rows.Count - 1
                ...
                rcOleDbCommand.Parameters.Add("@paraStrHth", OleDbType.VarChar, 30).Value = rcDataset.Tables("rc_rkdnr").Rows(i).Item("hth")
...
#Region "打印设置、打印、打印预览事件"
    ...
    Private Sub PreparePrintData()
        ...
        '套打
        'rcRps.PaperType = 1
        rcRps.Text(-1, 1) = g_PrnDwmc & "产品入库单"
        rcRps.Text(-1, 2) = "单据号：" & Trim(Me.TxtDjh.Text) & "  %p/%t"
        rcRps.Text(-1, 3) = "日期：" & Me.DtpRkrq.Value.ToLongDateString
        rcRps.Text(-1, 4) = "部门：(" & Me.TxtBmdm.Text & ")" & Me.LblBmmc.Text
        rcRps.Text(-1, 5) = "库存地点：(" & Me.TxtCkdm.Text & ")" & Me.LblCkmc.Text
        rcRps.Text(-1, 6) = "经手人：(" & Me.TxtZydm.Text & ")" & Me.LblZymc.Text
        Dim i As Integer
        Dim j As Integer
...
#Region "退出表单事件"
    ...
    'Private Sub FrmOeRkdSrz2_Closing(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles MyBase.Closing
    '    If IsEditing Then
...
#Region "参照录入"
    ...
    Private Sub BtnReference_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnReference.Click
        '调用表单
        Dim rcFrm As New FrmOeRkdReference
        With rcFrm
