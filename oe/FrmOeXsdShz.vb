...
#Region "窗体初始化"
    ...
    Private Sub FrmOeXsdShz_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
        '设置DataGridView
        ...
        '    rcOleDbCommand.CommandText = "SELECT paradblvalue FROM rc_para WHERE dwdm = ? AND paraid = '套打格式打印送货单' ORDER BY paraid"
...
#Region "显示产品送货单"
    ...
    Private Sub ShowXsd(ByVal xsdDjh As String)
        ...
        If rcDataset.Tables("rc_xsdml").Rows(0).Item("zydm").GetType.ToString <> "System.DBNull" Then
            Me.TxtZydm.Text = Trim(rcDataset.Tables("rc_xsdml").Rows(0).Item("zydm"))
        End If
        ...
        Try
            ...
            rcOleDbCommand.CommandTimeout = 300
            rcOleDbCommand.CommandType = CommandType.Text
            rcOleDbCommand.CommandText = "SELECT oe_xsd.cpdm,COALESCE(oe_xsd.cpmc,'') as cpmc,COALESCE(oe_xsd.dw,'') as dw,COALESCE(oe_xsd.hth,'') AS hth,oe_xsd.khddh,oe_xsd.khlh,oe_xsd.pihao,oe_xsd.mjsl,oe_xsd.sl,COALESCE(oe_xsd.dw,'') as dw,oe_xsd.fzsl,oe_xsd.fzdw,oe_xsd.dj,oe_xsd.hsdj,oe_xsd.je,oe_xsd.shlv,oe_xsd.se,oe_xsd.je + oe_xsd.se AS jese,oe_xsd.xsmemo,oe_xsd.dddjh,oe_xsd.ddxh FROM oe_xsd WHERE (oe_xsd.djh = ?) ORDER BY oe_xsd.xh"
            rcOleDbCommand.Parameters.Clear()
            rcOleDbCommand.Parameters.Add("@djh", OleDbType.VarChar, 15).Value = xsdDjh
            rcOleDbDataAdpt.SelectCommand = rcOleDbCommand
...
#Region "审核"
    ...
    Private Sub ShEvent(ByVal xsdDjh As String)
        Try
            ...
            rcOleDbCommand.CommandTimeout = 300
            rcOleDbCommand.CommandType = CommandType.Text
            rcOleDbCommand.CommandText = "UPDATE oe_xsd SET shr = ?,shrq = SYSDATE WHERE djh = ? AND shr IS NULL"
            rcOleDbCommand.Parameters.Clear()
...
#Region "消审"
    ...
    Private Sub XsEvent(ByVal xsdDjh As String)
        Try
            ...
            rcOleDbCommand.CommandTimeout = 300
            rcOleDbCommand.CommandType = CommandType.Text
            rcOleDbCommand.CommandText = "UPDATE oe_xsd SET shr = ?,shrq = SYSDATE WHERE djh = ? AND jzr IS NULL"
            rcOleDbCommand.Parameters.Clear()
