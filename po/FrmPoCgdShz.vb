...
#Region "窗体初始化"
    ...
    Private Sub FrmCgdShz_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
        '设置DataGridView
...
#Region "显示入库单"
    ...
    Private Sub ShowCkd(ByVal ckDjh As String)
        ...
        Try
            ...
            rcOleDbCommand.CommandTimeout = 300
            rcOleDbCommand.CommandType = CommandType.Text
            rcOleDbCommand.CommandText = "SELECT po_cgd.cpdm,po_cgd.cpmc,po_cgd.jhshrq,po_cgd.sl,po_cgd.dw,po_cgd.mjsl,po_cgd.fzsl,po_cgd.fzdw,po_cgd.dj,po_cgd.hsdj,po_cgd.je,po_cgd.shlv,po_cgd.se,(po_cgd.je+ po_cgd.se) AS jese,po_cgd.cgmemo,po_cgd.cgjhdjh,po_cgd.cgjhxh,po_cgd.rksl,po_cgd.rkje FROM po_cgd WHERE po_cgd.djh = ? ORDER BY po_cgd.xh"
            rcOleDbCommand.Parameters.Clear()
            rcOleDbCommand.Parameters.Add("@djh", OleDbType.VarChar, 15).Value = ckDjh
            rcOleDbDataAdpt.SelectCommand = rcOleDbCommand
...
#Region "计算合计数"
    ...
    Private Sub SumSlJe()
        ...
        If rcDataSet.Tables("rc_cgdnr").Rows.Count > 0 Then
            ...
            dblTotSe = rcDataSet.Tables("rc_cgdnr").Compute("Sum(se)", "")
        End If
...
#Region "审核"
    ...
    Private Sub ShEvent(ByVal ckDjh As String)
        Try
            ...
            rcOleDbCommand.CommandTimeout = 300
            rcOleDbCommand.CommandType = CommandType.Text
            rcOleDbCommand.CommandText = "UPDATE po_cgd SET shr = ?,shrq = SYSDATE WHERE djh = ?"
            rcOleDbCommand.Parameters.Clear()
...
#Region "消审"
    ...
    Private Sub XsEvent(ByVal ckDjh As String)
        Try
            ...
            rcOleDbCommand.CommandTimeout = 300
            rcOleDbCommand.CommandType = CommandType.Text
            rcOleDbCommand.CommandText = "UPDATE po_cgd SET shr = ?,shrq = NULL WHERE NOT EXISTS (SELECT 1 FROM po_cgd a WHERE a.rksl <> 0 AND a.djh = ?) AND djh = ?"
            rcOleDbCommand.Parameters.Clear()
...
#Region "准备打印数据事件"
    ...
    Private Sub PreparePrintData()
        ...
        End If
        Dim rft As String =Application.StartupPath + "\reports\cgdbz.rft"
