...
Public Class FrmPoLlsqSh
    ...
    Private Sub FrmPoLlsqSh_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        '默认值
...
#End Region
    ...
    Private Sub BtnOk_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnOk.Click
        ...
        Try
            ...
            rcOleDbCommand.CommandTimeout = 300
            rcOleDbCommand.CommandType = CommandType.Text
            rcOleDbCommand.CommandText = "SELECT inv_llsq.djh FROM inv_llsq WHERE SUBSTR(inv_llsq.djh,11, 5) >= ?  AND SUBSTR(inv_llsq.djh,11, 5) <= ?" & IIf(ChbSh.Checked, " AND inv_llsq.shr IS NULL AND inv_llsq.bclosed ＝ 0", " AND inv_llsq.sqrq >= ? AND inv_llsq.sqrq <= ?") & IIf(Not String.IsNullOrEmpty(Me.TxtBmdm.Text), " AND inv_llsq.bmdm LIKE '" & Trim(Me.TxtBmdm.Text) & "%'", "") & IIf(Me.CmbPzlxjc.SelectedValue <> "0000", " AND SUBSTR(inv_llsq.djh,1, 4) ='" & Me.CmbPzlxjc.SelectedValue & "'", "") & " GROUP BY inv_llsq.djh ORDER BY inv_llsq.djh"
            rcOleDbCommand.Parameters.Clear()
            ...
            If Not ChbSh.Checked Then
                rcOleDbCommand.Parameters.Add("@sqrq", OleDbType.Date, 8).Value = Me.DtpBegin.Value
                rcOleDbCommand.Parameters.Add("@sqrq", OleDbType.Date, 8).Value = Me.DtpEnd.Value
            End If
            rcOleDbDataAdpt.SelectCommand = rcOleDbCommand
            If rcDataSet.Tables("llsqml") IsNot Nothing Then
                rcDataSet.Tables("llsqml").Clear()
            End If
