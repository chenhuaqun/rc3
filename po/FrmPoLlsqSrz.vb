                        ...
                        If rcDataset.Tables("po_rkd") IsNot Nothing Then
                            If rcDataset.Tables("po_rkd").Rows.Count > 0 Then
                                If rcDataset.Tables("po_rkd").Rows(0).Item("csdm").GetType.ToString <> "System.DBNull" Then
                                    ...
                                    '保存当前供应商库存
                                    dblCsKcsl = ReadCsKcsl(rcDataGridView.Rows(rcDataGridView.CurrentRow.Index).Cells("ColCpdm").EditedFormattedValue, rcDataGridView.Rows(rcDataGridView.CurrentRow.Index).Cells("ColCsdm").Value, rcDataset.Tables("rc_cpxx").Rows(0).Item("ckdm"), Me.TxtDjh.Text)
            ...
            If Me.rcDataGridView.Columns(e.ColumnIndex).Name = "ColCsdm" Then
                If Not String.IsNullOrEmpty(e.FormattedValue) Then
                    ...
                    If rcDataset.Tables("rc_csxx").Rows.Count > 0 Then
                        ...
                        dblCsKcsl = ReadCsKcsl(rcDataGridView.Rows(rcDataGridView.CurrentRow.Index).Cells("ColCpdm").Value, rcDataGridView.Rows(rcDataGridView.CurrentRow.Index).Cells("ColCsdm").EditedFormattedValue, rcDataset.Tables("rc_cpxx").Rows(0).Item("ckdm"), Me.TxtDjh.Text)
                        Me.LblMsg.Text = "仓库库存数量：" & Format(dblCkKcsl, g_FormatSl) & " 该供应商库存数量：" & Format(dblCsKcsl, g_FormatSl)
                    Else
...
#Region "准备打印数据事件"
    ...
    Private Sub PreparePrintData()
        ...
        End If
        Dim rft As String =Application.StartupPath + "\reports\ckdbz.rft"
