...
Public Class FrmCsdmGg
    ...
    Private Sub TxtOldCsdm_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles TxtOldCsdm.Validating
        If Not String.IsNullOrEmpty(Me.TxtOldCsdm.Text) Then
            ...
            If rcDataSet.Tables("oldcsxx").Rows.Count > 0 Then
                ...
                End If
            Else
                MsgBox("该供应商编码不存在。", MsgBoxStyle.OkOnly + MsgBoxStyle.Question, "提示信息")
                TxtOldCsdm.Focus()
            End If
        End If
    End Sub
    ...
    Private Sub BtnOk_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnOk.Click
        ...
        If Me.TxtOldCsdm.Text = Me.TxtNewCsdm.Text Then
            MsgBox("供应商编码相同，不需要更改。", MsgBoxStyle.OkOnly + MsgBoxStyle.Information, "提示信息")
            Return
        End If
        If rcDataSet.Tables("newcsxx").Rows.Count > 0 Then
            If Not MsgBox("新供应商编码已经存在。您是否要合并供应商编码？", MsgBoxStyle.YesNo + MsgBoxStyle.Information + MsgBoxStyle.DefaultButton2, "提示信息") = MsgBoxResult.Yes Then
                TxtNewCsdm.Text = ""
                TxtNewCsmc.Text = ""
                TxtNewCsdm.Focus()
                Return
        ...
        Try
            ...
            'rcOleDbCommand.CommandText = "SELECT kjnd,kmdm,bmdm,zydm,xmdm,khdm,yhzh,jxzh,Sum(CASE WHEN jd = '借' THEN Nvl(ncsl,0.0) ELSE 0 - Nvl(ncsl,0.0) END) as ncsl,Sum(CASE WHEN jd = '借' THEN Nvl(ncwb,0.0) ELSE 0 - Nvl(ncwb,0.0) END) as ncwb,Sum(CASE WHEN jd = '借' THEN Nvl(ncje,0.0) ELSE 0 - Nvl(ncje,0.0) END) as ncje FROM gl_kmyeb WHERE (csdm = ? or csdm = ?) GROUP BY kjnd"
            '科目余额表
            rcOleDbCommand.CommandText = "SELECT kjnd,kmdm,wbdm,bmdm,zydm,xmdm,khdm,yhzh,jxzh,'借' AS jd,SUM(CASE WHEN jd ='借' THEN ncsl ELSE 0 - ncsl END) AS ncsl,SUM(CASE WHEN jd ='借' THEN ncwb ELSE 0 - ncwb END) AS ncwb,SUM(CASE WHEN jd ='借' THEN ncje ELSE 0 - ncje END) AS ncje" &
        ...
        Finally
            rcOleDbConn.Close()
        End Try
        MsgBox("更改与合并供应商编码完成。" & Chr(13) & "请检查库存数量及金额、应收、应付等的数据正确性。", MsgBoxStyle.OkOnly + MsgBoxStyle.Question, "提示信息")
        Me.TxtOldCsdm.Text = ""
        Me.TxtNewCsdm.Text = ""
        Me.TxtOldCsmc.Text = ""
        Me.TxtNewCsmc.Text = ""
